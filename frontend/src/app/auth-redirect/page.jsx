...
export default function AuthRedirectPage() {
    ...
    const checkUser = async () => {
      try {
        ...
        if (userData.status === 0) {
          if (userData.reason === 1) return router.push(`/auth-redirect/${1}`);
          if (userData.reason === 2) return router.push(`/auth-redirect/${2}`);
          if (userData.reason === 3) return router.push(`/auth-redirect/${3}`);
        }

        if (userData.status === 1) return router.push("/register");

        if (userData.status === 2) {
          console.log("user", userData)
          if (userData.user.is_superuser) return router.push("/adminHome");
          else return router.push("/authHome");
        }
  ...
  return (
        <>
            <div className="main-container-auth-redirect">
                <div className="container-auth-redirect">
                    <div className="info-container-ARG">
